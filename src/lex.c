...
static void print_token(LexToken_T *token) {
    ...
    if (token->type == TOKEN_INTEGER || token->type == TOKEN_CHARACTER_LITERAL) {
        printf("{TYPE: %d | WORD: %lld | LINE: %d}\n", token->type, token->ival, token->lineno); 
    } else if (token->type == TOKEN_FLOAT) {
        printf("{TYPE: %d | WORD: %.3f | LINE: %d}\n", token->type, token->fval, token->lineno); 
    } else {
...
static void read_operator(LexState_T *L) {
    ...
    for (int i = 0; i < sizeof(multi_operators)/sizeof(char *); i++) {
        const char *operator = multi_operators[i];
    ...
    make_token(L, TOKEN_OPERATOR, opbuf);
...
static LexState_T *init_lexstate(const char *filename) {
    ...
    assert(L->contents);
    fread(L->contents, 1, L->flen, infile);
    L->contents[L->flen] = 0;
    fclose(infile);
